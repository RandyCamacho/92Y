...
extension DatabaseManager {
    ...
    public func getAllChatsForEmail(for email: String, completion: @escaping (Result<[Conversation], Error>) -> Void) {
    ...
    public func getAllMessagesForChat(with id: String, completion: @escaping (Result<[Message], Error>) -> Void) {
        database.child("\(id)/messages").observe(.value, with: { snapshot in
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                ...
                return Message(sender: sender, kind: .text(content), messageId: messageId, sentDate: date)
            })
